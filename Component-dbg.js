...
sap.ui.define([
	...
	return UIComponent.extend("i2d.mpe.operations.manages1.Component", {
		...
		init: function() {
            ...
            //this model will be having a data, ,that cab be used later in the Blocks to apply filters on the smart table.
			...
			if (sPatternParam) {
				...
				if (sPatternParam.indexOf("C_Manageoperations") > -1 && sPatternParam.indexOf("sap-iapp-state") > -1) {
					this._oRouter.initialize("true");
					this.addAppStateKey("object",sPatternParam.split("/")[1]);
				} else if(sPatternParam.indexOf("C_Manageoperations") === -1 && sPatternParam.indexOf("sap-iapp-state") > -1) {
					this._oRouter.initialize("true");
		...
		_crossAppNavToDetail: function(sView, oRouter, sOperationId, oHashChanger) {
			// handle the case that App was reached via Cross App navigation
			var sUrl;
			if(sView === "worklist"){
				sUrl = oRouter.getURL("worklist", {
        ...
        setOperationDetailModel: function(){
            var oOperationDetailModel = new sap.ui.model.json.JSONModel({});
            var oOperationDetailData = oOperationDetailModel.getData();
			oOperationDetailData.bEnableAutoBinding = true;
			oOperationDetailModel.setData(oOperationDetailData);
            this.setModel(oOperationDetailModel, "DetailModel");
        },
		/**
		...
		intializeAppState: function() {
			...
			// Model creation
			//Create and register models prior to the createContent method invocation
			...
			// The Application state key to pass information to the called applications
			...
			// now that the key is known,  calculate the CrossApplication links
			this.calculateCrossAppLinks(); //because the same key for the whole session ,need to initialize it only once
			...
			// This must be done *after* it is processed a potential inner app state from initial invocation (thus the promise)
		...
		extractInnerAppStateFromURL: function(sInnerAppStateKey, sCurrentRouteName, sParameters) {
			...
			// A distinct appstate key -> generate a new model
			that.createANewAppStateModel();
			// Must apply the inner App State *after* treating CrossAppState (x-app-state)
		...
		createANewAppStateModel: function() {
			...
			// key is available, calculate the CrossApplication links
			this.calculateCrossAppLinks(); //recalculate the model as the links are updated
		...
		updateModelFromAppstate: function(oModel, oAppState) {
			...
			if (oAppStateData) {	var sPattern = this.getRouter().oHashChanger.getHash();
				if (sPattern.indexOf("C_Manageoperations") > -1) {
					sap.ui.getCore().getEventBus().publish("AppState", "hanldeAppstateDetailChanges", oAppStateData.detailPage);
				} else {
					sap.ui.getCore().getEventBus().publish("AppState", "handleAppstateChanges", oAppStateData);
				}
				return true;
			}
			return false;
